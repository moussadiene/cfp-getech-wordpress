...
<div class="tempo-meta top single">
    <ul>
        <?php
            // author
            if( $show_author )
                echo '<li><a class="author" href="' . esc_url( get_author_posts_url( $post-> post_author ) ) . '" title="' . sprintf( __( 'Posted by %1s' , 'gardener' ) , esc_attr( $name ) ) . '">' . esc_html( get_the_author_meta( 'display_name' , $post -> post_author ) ) . '</a></li>';
            ...
            if( $show_time ){
                ...
                echo '<li><a href="' . esc_url( get_day_link( $y , $m , $d ) )  . '" title="' . sprintf( __( 'posted on %1s', 'gardener' ), $ptime ) . '"><time datetime="' . esc_attr( $dtime ) . '">' . esc_html( $ptime ) . '</time></a></li>';
            }
